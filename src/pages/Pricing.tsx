      ...
      <main className="mx-auto max-w-5xl px-6 py-12 text-center">
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-3">
          {pricingPlans.map((plan) => (
            <div key={plan.id} className={cn('relative flex flex-col rounded-3xl border p-7 text-left transition-transform hover:-translate-y-1', plan.highlighted ? 'border-brand-500 bg-surface-0 shadow-lift dark:bg-dark-surface' : 'border-line bg-surface-0 shadow-softer dark:border-dark-border dark:bg-dark-surface')}>
              {plan.highlighted && <span className="absolute -top-3 left-7 rounded-full bg-brand-500 px-3 py-1 text-xs font-semibold text-white">Most popular</span>}
