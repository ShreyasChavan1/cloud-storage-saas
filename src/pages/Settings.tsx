...
export default function Settings() {
  ...
  const [removingCctvId, setRemovingCctvId] = useState<string | null>(null)
          ...
          {active === 'security' && (
            <Card className="p-6">
              ...
              <div className="mt-7 rounded-xl border border-line p-4 dark:border-dark-border">
                ...
                <Button variant="secondary" size="sm" className="mt-3" loading={passwordSaving} disabled={!passwords.current} onClick={async () => {
              ...
              <div className="mt-6 space-y-3">
                {cctvDevices.map(d => (
                  <div key={d.id} className="rounded-xl border border-line p-4 dark:border-dark-border">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{d.name}</p><p className="text-xs text-ink-500">{d.status !== 'ACTIVE' ? '● Waiting for gateway enrollment' : d.lastSeenAt && Date.now() - new Date(d.lastSeenAt).getTime() < 120000 ? '● Gateway online' : '● Gateway offline'} · {d.camerasConfigured ?? 0} camera(s) · {d.lastUploadAt ? `last upload ${new Date(d.lastUploadAt).toLocaleString()}` : 'no uploads yet'}</p></div><div className="flex gap-2"><Button variant="secondary" size="sm" disabled={removingCctvId === d.id} onClick={() => { setEditingCctvId(d.id); setNewCctvEnrollment(null); setCctvConfig({ nvrHost: d.nvrHost ?? '', nvrUsername: d.nvrUsername ?? '', nvrPassword: '', segmentSeconds: d.segmentSeconds ?? 300, uploadPollSeconds: d.uploadPollSeconds ?? 15 }); setCctvCameras([{ name: 'Camera 01', rtspUrl: '', enabled: true }]) }}><RefreshCw className="h-3.5 w-3.5" />{d.status === 'ACTIVE' ? 'Reconfigure' : 'Configure'}</Button><Button variant="danger" size="sm" loading={removingCctvId === d.id} onClick={async () => { if (!window.confirm(`Remove ${d.name}?`)) return; setRemovingCctvId(d.id); try { await cctvApi.remove(d.id); setCctvDevices(x => x.filter(i => i.id !== d.id)); if (newCctvEnrollment?.id === d.id) setNewCctvEnrollment(null); if (editingCctvId === d.id) setEditingCctvId(null); showToast('CCTV gateway removed.') } catch (err) { showToast(getErrorMessage(err, 'Could not remove CCTV gateway.'), 'error') } finally { setRemovingCctvId(null) } }}>Remove</Button></div></div>
                  </div>
                ))}
