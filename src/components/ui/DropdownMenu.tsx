...
export interface MenuItemDef { label: string; icon: ReactNode; onSelect: () => void; tone?: 'default' | 'danger'; disabled?: boolean }
...
export function DropdownMenu({ trigger, items, align = 'end' }: DropdownMenuProps) {
  ...
  const openMenu = () => {
    ...
    if (rect) setCoords({ top: rect.bottom + window.scrollY + 6, left: align === 'end' ? rect.right + window.scrollX - 180 : rect.left + window.scrollX })
    setOpen((o) => !o)
  }
  return (
    <>
      ...
      {open && createPortal(
        <div ref={menuRef} style={{ top: coords.top, left: coords.left }} className="fixed z-50 w-44 overflow-hidden rounded-xl border border-line bg-surface-0 py-1 shadow-soft animate-fade-up dark:border-dark-border dark:bg-dark-surface2">
          {items.map((item) => (
